...
本系统采用浏览器原生的表单校验（如 `required`、`type="email"`），不再使用自定义红色错误提示。这样可以有效减弱用户对错误提示的抵触情绪，避免满屏红色带来的压力。浏览器只会在第一个未通过校验的字段弹出气泡提示，用户体验更友好，且实现更为简洁。后续如需更复杂的校验或自定义提示，可随时扩展。
...
**设计理念：** 采用浏览器原生校验机制，减少用户对错误提示的抵触情绪
